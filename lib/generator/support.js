...
export function* getTree(srcPath, destPath, excludes, templateLocals, mode, currentPath = ".") {
  ...
  for (let file of files) {
    ...
    if (file.stats.isDirectory()) yield* getTree(srcPath, destPath, excludes, templateLocals, mode, filePath);
...
function getMetadata(filePath, srcPath, destPath, templateLocals, mode) {
  ...
  let compiledData = !stats.isDirectory() && isGenerate(mode) &&
    compileTemplate(fs.readFileSync(templatePath), templateLocals);

  return { templatePath, outputPath, stats, compiledData };
}
...
export function writeFile(file, overwrite) {
  fs.writeFileSync(file.outputPath, file.compiledData, { mode: file.stats.mode });
  logAction(overwrite ? FORCE : CREATE, file.outputPath);
}
...
export function showDiff(oldStr, newStr) {
  ...
  let diff = parts.reduce((str, part) => str.concat(
    part.value.replace(/[^\n]*\n/g, line => (
