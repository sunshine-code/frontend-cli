...
const STYLELINT_EXTERNAL_CONFIG = exists(resolveExternal(".stylelintrc"));
const STYLELINT = stylelint({
  configFile: !STYLELINT_EXTERNAL_CONFIG && resolveInternal("templates", "linters", "stylelint"),
});
...
export default (options) => [
  new LoaderOptionsPlugin({
    ...
    options: {
      ...
      output: { path: resolveExternal(OUTPUT_PATH) },
      context: process.cwd()
    }
  })
];
